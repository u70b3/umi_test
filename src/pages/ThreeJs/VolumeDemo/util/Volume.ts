...
Volume.prototype = {
  constructor: Volume,
  //TODO:配合项目改变了数据读取方式，故此文件无法用于其他项目！
  /**
   * @member {Function} getData Shortcut for data[access(i,j,k)]
   * @memberof Volume
   * @param {number} i    Third coordinate
   * @param {number} j    Second coordinate
   * @param {number} k    First coordinate
   * @returns {number}  value in the data array
   */
  getData: function(i, j, k) {
    //return this.data[ i * this.yLength * this.zLength + j * this.zLength + k ];
    return this.data[k * this.xLength * this.yLength + j * this.xLength + i];
  },

  //TODO:这是原来文件内没有的函数
  setData: function(i, j, k, value) {
    this.data[k * this.xLength * this.yLength + j * this.xLength + i] = value;
  },

  //TODO:这是原来文件内没有的函数，用于对体素立方体进行“腐蚀”
  erodeData: function() {
    for (var i = 0; i < this.xLength; i++)
      for (var j = 0; j < this.yLength; j++)
        for (var k = 0; k < this.zLength; k++) {
          if (
            i === 0 ||
            j === 0 ||
            k === 0 ||
  ...
  access: function(i, j, k) {
    //return  i * this.yLength * this.zLength + j * this.zLength + k;
  ...
  /**
   * @member {Function} extractPerpendicularPlane Compute the orientation of the slice and returns all the information
   * relative to the geometry such as sliceAccess, the plane matrix (orientation and position in RAS coordinate)
